...
echo "========== Copying web.sh =========="
cp -f /home/ec2-user/3tier_aws_packer_project.git/application_code/web.sh /home/ec2-user/web.sh
...
sed -i "s|REPLACE-WITH-INTERNAL-LB-DNS|${app_alb_dns}|g" \
    /home/ec2-user/3tier_aws_packer_project/application_code/nginx.conf

# Backup old config & apply new one
mv /etc/nginx/nginx.conf /etc/nginx/nginx-backup.conf || true
cp -f /home/ec2-user/3tier_aws_packer_project/application_code/nginx.conf /etc/nginx/nginx.conf
